...
# Modified by Caroline Freyer <carolinefreyer@gmail.com>.
...
class RDFNMFPlus:
    ...
    def train(
        self, ratings_train, ratings_ours, validate_ratings=None, show_process_rmse=True
    ):
        ...
        for epoch in range(self.n_epochs):
            ...
            for (ext_user_id, ext_item_id, r) in ratings_train:
                ...
                reg_bias = (
                    ...
                    else (self.lmbda_bias / np.sqrt(self.n_item_rating[i] + self.alpha))
                    if self.method == "sqrt"
                    else (self.lmbda_bias / np.log(self.n_item_rating[i] + self.alpha))
                )
            ...
            for u in range(self.n_users_train):
                user_denom[u, :] += (
                    self.n_user_rating[u] * reg_latent_user[u] * self.P[u, :]
                )
            ...
            for i in range(self.n_items):
                ...
                try:
                    item_denom[i, :] += (
                        self.n_item_rating[i] * reg_latent_movie[i] * self.Q[i, :]
                    )
            ...
            for u in range(self.n_users_train, self.n_users_our + self.n_users_train):
                user_denom[u, :] += (
                    self.n_user_rating[u]
                    * (
                        reg_latent_user[u]
                        + self.lmbda_latent
                        * self.activity_level(u, user_variances, user_means)
                    )
                    * self.P[u, :]
                )
                self.P[u, :] *= user_num[u, :] / user_denom[u, :]
            ...
            if show_process_rmse:
                if validate_ratings is None:
                    ...
                    print(
                        "After %i epochs, loss=%.6f, training rmse=%.6f"
                        % (epoch + 1, loss, rmse)
                    )
                else:
        ...
        for (ext_user_id, ext_item_id, r) in user_item_pairs:
            all_predicts.append(
                (
                    ext_user_id,
                    ext_item_id,
                    self.predict_single_rating(ext_user_id, ext_item_id),
                )
            )
        return all_predicts
    ...
    def _compute_err(self, ratings):
        ...
        for (ext_user_id, ext_item_id, r) in ratings:
            ...
            sse += err_square
            loss += err_square
        loss += self.lmbda_latent * (
            np.linalg.norm(self.P) + np.linalg.norm(self.Q)
        ) + self.lmbda_bias * (np.linalg.norm(self.bu) + np.linalg.norm(self.bi))
        return loss, (sse / len(ratings)) ** 0.5

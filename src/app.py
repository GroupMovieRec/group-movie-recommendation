...
def get_popular_movies():
    ...
    with open(POPULAR_MOVIES_IDS_PATH, "r") as f:
        ...
        for id in list_popular_movies_ids:
            ...
            popular_movies = popular_movies.append(
                {
                    "movieId": row["movieId"].values[0],
...
def get_scores(reclist, movieIds):
    ...
    for id, score in zip(list(reclist["mIds"]), list(reclist["mScores"])):
        titles.append(
            f"{count}. {merged_a_b.loc[merged_a_b['movieId']==id].title.values[0]}"
        )
...
def recommendation_mf():
    ...
    recommendation = get_scores(
        reclist, list(merged_a_b[merged_a_b.movieId.isin(mIds)].movieId)
    )
    ...
    if request.method == "POST":
        ...
        if "run-mf-model" in request.form:
            ...
            session["movieIds"] = list(
                merged_a_b[merged_a_b.movieId.isin(random_merged_a_b)].movieId
            )
            session["top15"] = list(
                merged_a_b[merged_a_b.movieId.isin(random_merged_a_b)].title
            )
            session["top15_posters"] = list(
                merged_a_b[merged_a_b.movieId.isin(random_merged_a_b)].posterUrl
            )
        ...
        # Collect friends info
            ...
            with open("data_tu_delft/user_credentials.csv", "w") as f:
                ...
                for i in range(session["members"]):
                    ...
                    row = [
                        f"{i}",
                        f"{number_to_gender(session['userGenders'][-1])}",
                        f"{session['userFeelings'][-1]}",
                        f"{session['userAges'][-1]}",
                    ]
                    writer.writerow(row)
                f.close()
        ...
        # All people voting
            ...
            if session["counter"] == 0:
                shutil.copyfile(
                    "data_tu_delft/user_credentials.csv",
                    "data_tu_delft/previous_user_ratings.csv",
                )
            else:
                shutil.copyfile(
                    "data_tu_delft/user_ratings.csv",
                    "data_tu_delft/previous_user_ratings.csv",
                )
            ...
            add_column_in_csv(
                "data_tu_delft/previous_user_ratings.csv",
                "data_tu_delft/user_ratings.csv",
                lambda row, line_num: row.append(column[line_num - 1]),
            )
